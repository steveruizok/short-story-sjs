...
class ShortStoryNumberInput {
    ...
    render() {
        return [
            h("ssy-input-label", { label: this.label, propName: this.propName, type: "number" }),
            h("div", { class: "number" },
                h("input", { ref: node => (this.input = node), id: `stsy_input_${this.label}`, name: `stsy_input_${this.label}`, type: this.range ? 'range' : 'number', min: this.range ? this.min : null, max: this.range ? this.max : null, step: this.range ? this.step : null, onInput: this.inputChanged, value: this.value }),
                this.range || [
                    h("button", { style: { right: '44px' }, class: "input-button", onClick: ev => this.crementValue(ev, -1) }, "\u2212"),
                    h("button", { class: "input-button", onClick: ev => this.crementValue(ev, 1) }, "\uFF0B"),
                ]),
        ];
    }
